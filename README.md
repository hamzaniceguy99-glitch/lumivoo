# Lumivoo — writing coaching

Static marketing site (HTML / CSS / JS, no build step) hosted on GitHub Pages
at `lumivoo.shop`.

> Generated from `C:\Users\souha\coaching-sites-factory` (content file `sites/lumivoo.mjs`).
> To change the content, edit that file and run `node build.mjs lumivoo` — editing the
...
| 🟠 Important | `contact@lumivoo.shop` doesn't exist yet: set up free email forwarding in Namecheap (*Domain List → Manage → Redirect Email*). | Namecheap |
...
```
Coaching for writers, delivered online: one-on-one video sessions and small workshops covering finishing a first draft, story structure and outlining, revision and self-editing, and preparing work for submission to agents and publishers. Writers follow an 8 to 16 week program with weekly page submissions and written feedback from their coach. We are not a literary agency or publisher, charge no reading or submission fees, and guarantee no publication. Services are sold as month-to-month subscriptions from $39 to $199 per month, cancellable at any time. No physical products are sold or shipped. Site: lumivoo.shop
```
